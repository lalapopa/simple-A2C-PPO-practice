...
class Agent(BaseAgent):
    ...
    def __init__(self, load_path = None, **kwargs):
        ...
        if load_path is None:   
            ...
            if self.target_network:
                ...
                self.session.run([tf.compat.v1.assign(self.vars_tcritic[i], self.vars_critic[i]) for i in range(len(self.vars_critic))])
            else:
    ...
    def build_critic(self, learn_rate):
        ...
        # Placeholders
        self.x_critic = tf.compat.v1.placeholder(tf.float32, shape=[None, self.state_size], name='state_input')
        self.x_gradient_critic = tf.compat.v1.placeholder(tf.float32, shape=[None, self.state_size], name='gradient_input')
        self.x_lr_critic = tf.compat.v1.placeholder_with_default(learn_rate, shape=[], name='learn_rate_input')
        if self.reference_size > 0:
            self.x_ref_critic = tf.compat.v1.placeholder(tf.float32, shape=[None, self.reference_size], name='reference_input')
        ...
        with tf.compat.v1.variable_scope('critic'):
            ...
            self.output_critic = tf.compat.v1.layers.dense(x, self.state_size, activation=None, name='critic_output', **y_kwargs)
            ...
            self.optimizer_critic = tf.compat.v1.train.GradientDescentOptimizer(self.x_lr_critic)
        ...
        with tf.compat.v1.variable_scope('critic_target'):
            if self.target_network:
                ...
                for layer in range(len(self.hidden_layer_size)):
                    x = tf.compat.v1.layers.dense(x, self.hidden_layer_size[layer], name = 'dense_'+ str(layer), **h_kwargs)              
                self.output_tcritic = tf.compat.v1.layers.dense(x, self.state_size, activation=None, name='critic_output', **y_kwargs)
                self.y_tcritic = self.output_tcritic

                # Get target critic variables
                self.vars_tcritic = tf.compat.v1.trainable_variables(scope = 'critic_target')

                # Update operations
                self.optimize_tcritic = [tf.compat.v1.assign(self.vars_tcritic[i], self.tau*self.vars_critic[i] + (1-self.tau)*self.vars_tcritic[i]) for i in range(len(self.vars_critic))]
    ...
    def build_actor(self, learn_rate):
        ...
        with tf.compat.v1.variable_scope('actor'):
            # Placeholders
            self.x_actor = tf.compat.v1.placeholder(tf.float32, shape = [None, self.state_size], name = 'state_input')
            self.x_gradient_actor = tf.compat.v1.placeholder(tf.float32, shape = [None, self.output_size], name = 'gradient_input')
            self.x_lr_actor = tf.compat.v1.placeholder_with_default(learn_rate, shape=[], name='learn_rate_input')
            
            if self.reference_size > 0:
                self.x_ref_actor = tf.compat.v1.placeholder(tf.float32, shape=[None, self.reference_size], name='reference_input')
            ...
            for layer in range(len(self.hidden_layer_size)):
                ...
                x = tf.compat.v1.layers.dense(x, self.hidden_layer_size[layer], name=name_str, **h_kwargs)

            self.output_actor = [] 
            for action in range(self.output_size):
                self.output_actor.append(tf.compat.v1.layers.dense(x, 1, activation=None, name='actor_output_'+str(action), **y_kwargs))
            ...
            self.optimizer_actor = tf.compat.v1.train.GradientDescentOptimizer(self.x_lr_actor)
